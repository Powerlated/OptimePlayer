...
- `persisted.rs` — persistent user library (`TrackRef`, `Playlist`, `Library`, `RepeatMode`), the per-device `PerDeviceSettings` (resampling, stereo/mixer, shelf EQ, tuning — one each for `nds`/`gba`), and the `Persisted` bundle saved to eframe storage. All out-of-the-box defaults live in `Persisted::default` (the sub-structs intentionally have no `Default` impls). (`TrackRef::sseq_id` is the song id for any device; the field name is kept for storage compatibility.)
