...
- `media_controls.rs` — OS media-transport integration (`souvlaki`: Windows SMTC / macOS Now Playing / Linux MPRIS) so Bluetooth/keyboard media keys — e.g. AirPods double-tap to skip — drive the transport. Native impl gets the window `HWND` from the `eframe::Frame` (`raw-window-handle`), routes `MediaControlEvent`s through a channel into `MediaAction`s the app drains each frame (`handle_media_controls` in `app.rs`), and pushes the current title/playback state back as "now playing". The web build is a no-op stub.
