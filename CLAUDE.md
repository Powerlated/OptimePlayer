...
- `devices/mod.rs` — `SoundData` (enum over parsed archives: `load_all`, `song_ids` — GBA ids are filtered to playable songs only, `song_name`, `make_player`) and `DevicePlayer` (enum over running players: `clock_rate`, `cycles_per_tick`, `steps_elapsed`, `step_rate`, `tick`). Both enums box their variants.
