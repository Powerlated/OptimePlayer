...
- `devices/mod.rs` — `SoundData` (enum over parsed archives: `load_all`, `song_ids` — GBA ids are filtered to playable songs only, `song_name`, `make_player`) and `DevicePlayer` (enum over running players: `clock_rate`, `cycles_per_tick`, `steps_elapsed`, `step_rate`, `hardware_chain`, keyboard input, `tick`). Both enums box their variants.
- `devices/messages.rs` — the standardized stream: `SynthEvent` (`NoteStarted`, `VoiceVolume`, `VoicePitch`, `VoiceDetune`, `VoiceStopped`, `NoteReleased`, `TrackPan`, `TrackDetune`, `Looped`, `Ended`), `VoicePitch` (`Midi {note, sample_pitch_hz}` for repitched samples vs `DataRateHz` for register-driven rates), `VoiceId` (monotonic `u64`), `TickFeedback` (synth-side voice endings reported back to the device each tick), and `HardwareChain` (the device's declared fixed-rate output chain — GBA: 13379 Hz software mixer + 32768 Hz DAC; DS: 32768 Hz only — reproduced by `ResampleMode::Authentic`).
...
- `synth/` — `instrument.rs` (`SampleInstrument`, one voice: `VoicePitch`-driven frequency, resampling, pop-smoothing gain slew, block rendering), `authentic.rs` (the per-voice `HardwareChain` reproduction behind `ResampleMode::Authentic`: linear→mixer rate, NN-hold→DAC rate into a small ring, windowed-sinc to the output rate; PSGs NN at the DAC rate), `synthesizer.rs` (`SampleSynthesizer`: round-robin voice pool, pan, Haas widening with optional held delay changes, bass-mono crossover), `delay.rs` (`DelayLine`), `gather.rs` (SIMD/scalar sample gather).
- `resample/` — windowed-sinc resampling: `kernels.rs` (FIR kernel/response, `ResampleTables`), `mod.rs` (public API).
- `sample.rs` — `Sample`, `ResampleMode` (`NearestNeighbor`, `Linear`, clean `SincSampleNyquist`, crunchy `SincOutputNyquist` with per-kind PSG/sampler cutoff sliders, and `Authentic` with its own cutoff), + `decode_pcm8`/`decode_pcm16`/`decode_adpcm`/`decode_wav`.
...
- `app.rs` — `OptimeApp` (the `eframe::App`): song list/queue, transport, settings, WAV export, audio-only GBA ROM export, live keyboard input. Loads any `SoundData` source (`.nds`/`.sdat`/`.gba`). Resampling settings are **per device** (`nds_resample`/`gba_resample`, selected by the current song's console; only the active mode's sliders are shown). Demo loading is platform-split: native reads `demos/`, web fetches the `.sdat` at runtime (`web::fetch_bytes`). Shareable demo tracks via the URL query string on web.
