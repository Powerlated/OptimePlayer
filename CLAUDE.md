...
- **DSE (Procyon Studios) — in progress** — the SMDL (sequence) + SWDL (sample/instrument bank) formats used by *Pokémon Mystery Dungeon: Explorers of Sky*. The engine logic comes from the pmd-sky decompilation (https://github.com/pret/pmd-sky), cloned locally at `D:\Git\pmd-sky` (which also ships the extracted `files/SOUND/BGM/*.smd`/`*.swd` music). Currently **parsing + sample decoding only** (`devices/dse/`, proven by the `dump_dse` example); not yet a `SoundData`/`DevicePlayer` variant or a full playback backend.
