...
impl TrainProgress {
    /// New logger that emits at most every `interval_secs` (clamped to ≥50 ms).
    ...
    /// Per-epoch logger at the default ~1s cadence.
    pub fn per_epoch() -> Self {
        Self::new(DEFAULT_INTERVAL_SECS)
    }

    /// Print the current running-average loss + progress if the interval has
    /// elapsed, or if the epoch just finished (`n_batches >= n_total`). `running`
    /// is the sum of per-batch losses so far this epoch; `n_batches`/`n_total` are
    /// batches done / batches in the epoch; `tag` labels the line (e.g. `"ep 3"`).
    pub fn maybe_log(&mut self, tag: &str, running: f64, n_batches: usize, n_total: usize) {
        let now = Instant::now();
        ...
        println!(
            "  [{tag} {pct:>5.1}%] batch {n_batches}/{n_total}  loss {avg:.4}  {rate:.1} batch/s  {elapsed:.0}s"
        );
    }
}
...
mod tests {
    ...
    fn always_logs_when_epoch_done() {
        let mut p = TrainProgress::new(60.0); // interval long enough that time won't trigger
                                              // Even with a long interval, finishing the epoch must log.
        p.maybe_log("ep 1", 3.0, 3, 3);
    }

    #[test]
    fn per_epoch_default_interval_is_one_second() {
        let p = TrainProgress::per_epoch();
